...
- File fetch (LAN): `http://<host-ip>:8788/dataset/file?path=data/world/earth/nation_state/us/state_region/md/county_city/baltimore_city/layers/<file>.geojson.canonical.bin`
...
Defined in `sources/world/earth/nation_state/us/state_region/md/layers_manifest.json`. Runtime geometry loads from canonical layer binaries and compiled geometry artifacts, not stored layer GeoJSON. Manifest layer IDs may still end in `.geojson`, but that is an identifier convention rather than a runtime file requirement.
