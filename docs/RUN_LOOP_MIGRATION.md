...
- `app_frame_support.h/.cpp`: owns frame layout sizing, text-scale hotkey handling, and per-frame layer-pipeline progress snapshots.
...
- `layer_pipeline_drain.h/.cpp`: owns the per-frame layer-pipeline drain logic for completed background results.
...
- `layer_ui_state_sync.h/.cpp`: owns per-frame layer UI reconciliation, including changed-layer detection, newly-enabled layer work enqueueing, dependency coordination, and filter/UI state persistence.
...
- Layer download queue: inputs layer list, queue state, LAN peer context, freshness/status vectors, layer-work/local-file callbacks; outputs queue JSON, async download future state, status/freshness updates.
...
- Frame/pipeline support: inputs ImGui IO, panel fractions, font scale, atomic counts, queues, and timestamps; outputs frame layout plus layer-pipeline progress snapshots and idle timer updates.
...
- Layer pipeline drain: inputs completed layer-work queues, layer/state vectors, status/profile dirty state, request flags, counters, and a heap-trim callback; outputs applied layer updates and related state/counter updates.
...
- Left panel: inputs left-column layout, app settings, layer/freshness/runtime state, basemap cache state, parcel-jurisdiction filters, and download/layer-work callbacks; outputs the full `Layers and Controls` window plus visibility/filter mutations and missing-download summary counts for the data library window.
- Layer UI state sync: inputs layer enablement snapshots, changed UI flags, dependency/filter state, and layer-work callbacks; outputs updated persisted UI/filter state, refreshed last-enabled snapshots, and the derived vacant-layer-active flag.
...
- Right panel: inputs UI layout, parcel selection state, DuckDB/query/filter state, owner aggregate state, histogram/vacancy counters, layer-pipeline queue state, and tab callbacks; outputs the full `Record Filters` window plus parcel-selection and DuckDB rebuild side effects.
