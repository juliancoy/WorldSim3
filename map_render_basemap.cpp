...
MapBasemapRenderResult renderMapBasemap(const MapBasemapRenderContext& ctx) {
    ...
    int min_x = (int)std::floor((ctx.center_world.x - half_w_world) / 256.0) - 1;
    int max_x = (int)std::floor((ctx.center_world.x + half_w_world) / 256.0) + 1;
    int min_y = (int)std::floor((ctx.center_world.y - half_h_world) / 256.0) - 1;
    int max_y = (int)std::floor((ctx.center_world.y + half_h_world) / 256.0) + 1;
